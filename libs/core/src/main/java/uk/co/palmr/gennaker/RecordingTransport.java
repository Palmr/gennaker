...
/**
 * {@link Transport} decorator that records every {@link #publish} call before
 * forwarding it to a delegate. Intended for integration tests: wrap a
 * {@link DirectTransport} (or any other transport), wire the system under
 * test through the recorder, and assert on the captured traffic afterwards.
 *
 * <p>Recordings are kept as per-topic lists of encoded byte arrays, so tests
 * in a large topology can query just the topics they care about without
...
public final class RecordingTransport implements Transport {
     ...
     * @return an unmodifiable list of encoded message bodies in publish order,
     *         or an empty list if nothing was published on this topic
     */
    ...
    /**
     * Replays every recorded message for {@code topicClass} through the
     * generated subscriber proxy onto {@code topicImplementation}. The
     * implementation receives decoded method calls exactly as a live
     * subscriber would, allowing assertions to be written in terms of
     * topic methods rather than raw bytes.
     *
     * @param topicClass          the {@code @Topic}-annotated interface
